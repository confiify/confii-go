...
| `OSPS-QA-02.02` | **Met:** the published [v1.2.1 release](https://github.com/confiify/confii-go/releases/tag/v1.2.1) provides a checksummed SPDX 2.3 JSON SBOM for each of its five compiled archives. The [protected release run](https://github.com/confiify/confii-go/actions/runs/30172392339) validated each SBOM through bomctl and attached GitHub/Sigstore provenance. On 2026-07-26, all public assets were downloaded independently, their checksums matched, every SBOM round-tripped through bomctl, and every archive verified against the attached provenance bundle and expected release workflow identity. |
...
| `OSPS-QA-07.01` | **Met:** protected `main` requires a non-author code-owner approval, dismisses stale approvals, requires approval after the last push, and the public ledger in [`OPENSSF_BEST_PRACTICES.md`](OPENSSF_BEST_PRACTICES.md) records qualifying independent review for 14 of 28 non-trivial merged pull requests. |
