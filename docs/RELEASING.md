...
Create signed, annotated tags on the same verified commit. For `v1.2.1`:

```bash
git tag -s loader/cloud/v1.2.1 -m "loader/cloud v1.2.1"
git tag -s secret/cloud/v1.2.1 -m "secret/cloud v1.2.1"
git tag -s v1.2.1 -m "confii-go v1.2.1"
git push --atomic origin loader/cloud/v1.2.1 secret/cloud/v1.2.1 v1.2.1
```
...
gh release download v1.2.1 \
  --pattern 'confii-v1.2.1-linux-amd64.tar.gz' \
  --pattern 'confii-v1.2.1-linux-amd64.tar.gz.sbom.json' \
  --pattern 'checksums.txt' \
  --pattern '*.openvex.json' \
  --pattern 'confii-v1.2.1.intoto.jsonl'
sha256sum --check --ignore-missing checksums.txt
jq -e '.spdxVersion == "SPDX-2.3" and (.packages | length > 0)' \
  confii-v1.2.1-linux-amd64.tar.gz.sbom.json
gh attestation verify confii-v1.2.1-linux-amd64.tar.gz \
  --repo confiify/confii-go \
  --bundle confii-v1.2.1.intoto.jsonl \
  --signer-workflow confiify/confii-go/.github/workflows/release.yaml
go list -m github.com/confiify/confii-go@v1.2.1
go list -m github.com/confiify/confii-go/loader/cloud@v1.2.1
go list -m github.com/confiify/confii-go/secret/cloud@v1.2.1
```
